...
namespace IdentityModel.OidcClient.Pop
{
    ...
    {
        private readonly ILogger<EncodedList> Logger = null;

        public EncodedList(OidcClientOptions options, IEnumerable<string> keys, string hashValue)
        {
            ...
            options.LoggerFactory.CreateLogger<EncodedList>();
        }
        ...
        public bool IsSame(EncodedList other)
        {
            ...
            {
                Logger.LogDebug("Keys are not same length");
                return false;
            }

            if (HashedValue != other.HashedValue)
            {
                Logger.LogDebug("HashedValues do not match");
                return false;
            }
            ...
            for (var i = 0; i < Keys.Count(); i++)
            {
                if (arr1[i] != arr2[i])
                {
                    Logger.LogDebug("Values at position {0} do not match", i);
                    return false;
                }
            }
